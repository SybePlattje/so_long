...
/*   Updated: 2024/01/10 14:53:49 by splattje      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static char	**validate_and_split(char *all_lines, t_data *data)
{
	...
	if (all_lines[0] == '\0')
		return (ft_putendl_fd("Error\nEmpty map", 2), free(all_lines), NULL);
	index = 0;
	data->max_collectibles = 0;
	data->max_enemies = 0;
	while (all_lines[index] != '\0')
	{
		if (all_lines[index] == COLLECTIBLE)
			data->max_collectibles++;
		if (all_lines[index] == ENEMY)
			data->max_enemies++;
		if (all_lines[index++] == '\n' && all_lines[index] == '\n')
	...
	if (set_collectibles(data, new_lines))
		return (ft_putendl_fd("Error\nSetting collectibles", 2),
			free(all_lines), clean_up(data), NULL);
	return (new_lines);
}
...
int	check_map_vallid(char *file_name, t_data *data)
{
	...
	if (check_map_file(file_name))
		return (1);
	full_file_path = ft_strdup("./bonus/maps/");
